  ...
  return (
    <div className="min-h-screen bg-[#F2F2F7]">
      ...
      <header className="ios-navbar">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-3 flex items-center justify-between w-full">
          <Link href="/" className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-[#007AFF]/15 to-[#34C759]/15 border border-[#E5E5EA] flex items-center justify-center">
            ...
            <div>
              ...
              <p className="text-[10px] text-[#8E8E93]">Wholesaler Portal</p>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-4">
            ...
            {/* Notification bell */}
            <button className="relative w-11 h-11 rounded-lg bg-white border border-[#E5E5EA] flex items-center justify-center hover:bg-[#F2F2F7] transition-colors">
            ...
            <div onClick={logout} className="flex items-center gap-2 cursor-pointer hover:bg-[#FF3B30]/10 p-2 rounded-lg transition-colors group min-h-[44px]">
              <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-[#34C759]/20 to-[#5AC8FA]/20 border border-[#E5E5EA] flex items-center justify-center text-sm font-semibold text-[#248A3D] group-hover:border-[#FF3B30]/50 group-hover:text-[#FF3B30]">
                {user?.name?.charAt(0).toUpperCase() || "W"}
              </div>
              <div className="hidden md:block">
                <p className="text-sm font-medium text-[#000000] group-hover:text-[#FF3B30]">{user?.name || "Wholesaler"}</p>
                <p className="text-[10px] text-[#8E8E93] max-w-[150px] truncate">{user?.location || "Local Operations"}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-8">
        {/* Hero Section */}
        <div className="mb-8">
          <h1 className="ios-large-title text-[#000000] mb-2">
            Emergency Cargo Nearby
          </h1>
          <p className="text-[#8E8E93] ios-subheadline">
            {totalOffers > 0
        ...
        <div className="ios-segment w-fit mb-8">
          ...
          >
            My Orders
            <span className="ml-2 text-[#8E8E93]">({orders.length})</span>
          </button>
        </div>
          ...
          <>
            ...
            {emergencyCargos.length === 0 && upcomingCargos.length === 0 && (
              <div className="ios-card p-16 text-center">
                ...
                <h3 className="text-lg font-semibold text-[#000000] mb-2">No Distress Cargo Available</h3>
                ...
                <p className="text-xs text-[#8E8E93]/50 mt-4">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[#E5E5EA]">
                    <th className="text-left px-5 py-3 text-[10px] text-[#8E8E93] uppercase tracking-widest font-semibold">Cargo</th>
                    <th className="text-left px-5 py-3 text-[10px] text-[#8E8E93] uppercase tracking-widest font-semibold">Quantity</th>
                    <th className="text-left px-5 py-3 text-[10px] text-[#8E8E93] uppercase tracking-widest font-semibold">Price Paid</th>
                    <th className="text-left px-5 py-3 text-[10px] text-[#8E8E93] uppercase tracking-widest font-semibold">Total</th>
                    <th className="text-left px-5 py-3 text-[10px] text-[#8E8E93] uppercase tracking-widest font-semibold">Truck</th>
                    <th className="text-left px-5 py-3 text-[10px] text-[#8E8E93] uppercase tracking-widest font-semibold">Status</th>
                    <th className="text-left px-5 py-3 text-[10px] text-[#8E8E93] uppercase tracking-widest font-semibold">ETA</th>
                  </tr>
                </thead>
                <tbody>
                  {orders.map((order) => {
                    ...
                    <tr
                      key={order.id}
                      className="border-b border-[#E5E5EA] hover:bg-[#F2F2F7] transition-colors"
                    >
                      <td className="px-5 py-4">
                        <span className="text-sm font-medium text-[#000000] capitalize">
                          {cargo?.type || "Unknown"}
                        </span>
                      </td>
                      <td className="px-5 py-4">
                        <span className="font-[family-name:var(--font-mono)] text-sm text-[#3C3C43]">
                      ...
                      <td className="px-5 py-4">
                        <span className="font-[family-name:var(--font-mono)] text-sm text-[#000000] font-semibold">
                          ₹{order.totalValue.toLocaleString("en-IN")}
                        </span>
                      </td>
                      <td className="px-5 py-4">
                        <span className="font-[family-name:var(--font-mono)] text-xs text-[#8E8E93]">
                        ...
                        {order.status === "accepted" ? (
                          ...
                          <div className="flex flex-col items-center gap-1">
                            ...
                            <button 
                              ...
                              className="text-[9px] text-[#FF9500] hover:text-[#000000] underline mt-0.5"
                            >
                              Mark Paid
                            </button>
                          </div>
                ...
                return (
                  <div key={order.id} className="ios-card p-4">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-base font-semibold text-[#000000] capitalize">
                        {cargo?.type || "Unknown"}
                      </span>
                      {order.status === "accepted" ? (
                        <span className="font-[family-name:var(--font-mono)] text-xs font-bold text-[#FF9500] animate-pulse">
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-3">
                      <div>
                        <p className="text-[10px] text-[#8E8E93] uppercase tracking-widest">Qty</p>
                        <p className="font-[family-name:var(--font-mono)] text-sm text-[#3C3C43]">
                          {(order.requestedQuantityKg / 1000).toFixed(1)}T
                        </p>
                      </div>
                      <div>
                        <p className="text-[10px] text-[#8E8E93] uppercase tracking-widest">Price</p>
                      ...
                      <div>
                        ...
                        <p className="font-[family-name:var(--font-mono)] text-sm text-[#000000] font-semibold">
                          ₹{order.totalValue.toLocaleString("en-IN")}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center justify-between pt-3 border-t border-[#E5E5EA]">
                      ...
                      {order.status === "accepted" ? (
                        ...
                        <div className="flex flex-col items-end gap-1">
                          ...
                          <button 
                            onClick={() => dispatch({ type: "UPDATE_BID_STATUS", bidId: order.id, status: "payment_cleared" })}
                            className="text-[9px] text-[#FF9500] hover:text-[#000000] underline"
                          >
                            Mark Paid
                          </button>
                        </div>
